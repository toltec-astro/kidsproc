...
class TimeStream(FrequencyDivisionMultiplexingDataRef):
    ...
    def _slice_extra(self, item):
        ...
        if len(item) == 1:
            ...
            tone_slice, time_slice = item
        else:
            raise ValueError("too many slices.")
        result = super()._slice_extra(tone_slice)
    ...
    def __init__(
            self, tones=None, times=None, **kwargs):

        attrs_in_kwargs = set(kwargs.keys()).intersection(
        ...
        super().__init__(tones=tones, **kwargs)
        self._times = times
